    ...
    try:
        ...
        if form=="A":
            ...
            if FLUfs!=FRDfs or FLUfs!=BLDfs or FLUfs!=BRUfs:
                messagebox.showerror("Error", message="Signals sample frequency mismatch")
                btn_loadA["text"] = "Load A format"
                showhidecontrols(1)
                return
            if mestype=="sweep":
                ...
                if FLUfs!=InvFiltfs:
                    messagebox.showerror("Error", message="Inverse filter sample frequency and data sample frequency mismatch")    
                    btn_loadA["text"] = "Load A format"
                    showhidecontrols(1)
                    return
...
def func_analyze():
    try: 
        ...
        #IRstart = np.where(W2[0:np.argmax(W2)] <= np.max(W2) / 10)[0]
        #if len(IRstart) == 0:
        #   IRstart = 0
        #else:
        #    IRstart = IRstart[-1]
...
def func_expdata():
    # Export the intensity data to .Csv
    if len(graphxy.lines)==0:  
        messagebox.showerror("Error", message="No data analysed")
        return   
    filename = filedialog.asksaveasfilename(title="Export intensity data", filetypes=[("CSV File", "*.csv")],
...
lbl_winlen = Label(menu_frame, text="Win. lenght:")
...
btn_expdata = Button(menu_frame, text="Export intensity data", command=func_expdata)
...
btn_expplot.grid(row=13, column=0, columnspan=3, padx=3, pady=3, sticky="ewns")

lbl_pltselect = Label(menu_frame, text="Plot:")
lbl_pltselect.grid(row=14, column=0, padx=2, sticky="ens")

cmbx_pltselect = Combobox(menu_frame, values=("Hedgehog"), state="readonly", width=4)
cmbx_pltselect.grid(row=14, column=1, columnspan=2, pady=3, sticky="ewns")
cmbx_pltselect.current(0)
...
scale_rotatexy.grid(row=15, column=0, columnspan=3, pady=2, sticky="ewns")
scale_rotatexy.grid_remove()
...
scale_zoomxy.grid(row=16, column=0, columnspan=3, pady=2, sticky="ewns")
scale_zoomxy.set(1)
scale_zoomxy.grid_remove()

invertxy = IntVar()
chkbtn_invertxy = Checkbutton(menu_frame, text="Invert hedgehog", variable=invertxy, command=func_aspecthedgehogxy)
chkbtn_invertxy.grid(row=17, column=0, columnspan=3, padx=3, pady=2, sticky="ewns")
chkbtn_invertxy.grid_remove()
...
scale_rotatexz.grid(row=15, column=0, columnspan=3, pady=2, sticky="ewns")
scale_rotatexz.grid_remove()
...
scale_zoomxz.grid(row=16, column=0, columnspan=3, pady=2, sticky="ewns")
scale_zoomxz.set(1)
scale_zoomxz.grid_remove()

invertxz = IntVar()
chkbtn_invertxz = Checkbutton(menu_frame, text="Invert hedgehog", variable=invertxz, command=func_aspecthedgehogxz)
chkbtn_invertxz.grid(row=17, column=0, columnspan=3, padx=3, pady=2, sticky="ewns")
chkbtn_invertxz.grid_remove()
...
scale_rotateyz.grid(row=15, column=0, columnspan=3, pady=2, sticky="ewns")
scale_rotateyz.grid_remove()
...
scale_zoomyz.grid(row=16, column=0, columnspan=3, pady=2, sticky="ewns")
scale_zoomyz.set(1)
scale_zoomyz.grid_remove()

invertyz = IntVar()
chkbtn_invertyz = Checkbutton(menu_frame, text="Invert hedgehog", variable=invertyz, command=func_aspecthedgehogyz)
chkbtn_invertyz.grid(row=17, column=0, columnspan=3, padx=3, pady=2, sticky="ewns")
chkbtn_invertyz.grid_remove()
